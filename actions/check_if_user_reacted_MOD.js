module.exports = {
	...
	action: function (cache) {
		...
		if (reaction && reaction.users) {
			const array = String(reaction.users.cache.array());
			if (Array.isArray(member)) {
				result = member.every((member) => {
					return array.includes(String(member).replace(/!/g, ''));
				})
			} else if (member) {
				result = array.includes(String(member).replace(/!/g, ''));
			}
		}
